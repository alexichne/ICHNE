...
kpidf <- read.csv("Z:/Innovation_Lab/CT/data.csv")


```


```{r ngene, echo=FALSE, fig.show="hold"}

nkpi <- subset(kpidf, NgeneCt > 0)
ggplot(nkpi, aes(x=NgeneCt)) +
    geom_histogram(binwidth=1, color="black", fill  = "#1380A1")+
    labs(title="N Gene Ct distribution", subtitle = "Ct distribution of samples for previous 7 days", show_guide=F)+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()

```


```{r sgene, echo=FALSE, fig.show="hold"}

Skpi <- subset(kpidf, SgeneCt > 0)
ggplot(Skpi, aes(x=SgeneCt)) +
  geom_histogram(binwidth=1, color="black",fill = "#990000")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
      labs(title="S Gene Ct distribution", subtitle = "Ct distribution of samples for previous 7 days", show_guide=F)+
...
```{r orfgene, echo=FALSE, fig.show="hold"}

orfkpi <- subset(kpidf, Orf1abCt > 0)
ggplot(orfkpi, aes(x=Orf1abCt)) +
      labs(title="Orf1ab Ct distribution", subtitle = "Ct distribution of samples for previous 7 days", show_guide=F)+
  geom_histogram(binwidth=1, color="black",fill="#FAAB18")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r ngenep, echo=FALSE, fig.show="hold"}

nkpi <- subset(kpidf, NgeneCt > 0 & Result == "Positive")
ggplot(nkpi, aes(x=NgeneCt)) +
    geom_histogram(binwidth=1, color="black", fill  = "#1380A1")+
    labs(title="N Gene Ct distribution", subtitle = "Ct distribution of positive samples for previous 7 days", show_guide=F)+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r sgenep, echo=FALSE, fig.show="hold"}

Skpi <- subset(kpidf, SgeneCt > 0 & Result == "Positive")
ggplot(Skpi, aes(x=SgeneCt)) +
  geom_histogram(binwidth=1, color="black",fill = "#990000")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
      labs(title="S Gene Ct distribution", subtitle = "Ct distribution of positive samples for previous 7 days", show_guide=F)+
...
```{r orfgenep, echo=FALSE, fig.show="hold"}

orfkpi <- subset(kpidf, Orf1abCt > 0 & Result == "Positive")
ggplot(orfkpi, aes(x=Orf1abCt)) +
      labs(title="Orf1ab Ct distribution", subtitle = "Ct distribution of positive samples for previous 7 days", show_guide=F)+
  geom_histogram(binwidth=1, color="black",fill="#FAAB18")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r ngenem, echo=FALSE, fig.show="hold"}

nkpi <- subset(kpidf, NgeneCt > 0 &  substr(AdbOvenRack,1,2) == "BL")
ggplot(nkpi, aes(x=NgeneCt)) +
    geom_histogram(binwidth=1, color="black", fill  = "#1380A1")+
    labs(title="N Gene Ct distribution", subtitle = "Ct distribution of positive MedDX samples for previous 7 days", show_guide=F)+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r sgenem, echo=FALSE, fig.show="hold"}

Skpi <- subset(kpidf, SgeneCt > 0 &  substr(AdbOvenRack,1,2) == "BL")
ggplot(Skpi, aes(x=SgeneCt)) +
  geom_histogram(binwidth=1, color="black",fill = "#990000")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
      labs(title="S Gene Ct distribution", subtitle = "Ct distribution of positive MedDX samples for previous 7 days", show_guide=F)+
...
```{r orfgenem, echo=FALSE, fig.show="hold"}

orfkpi <- subset(kpidf, Orf1abCt > 0 &  substr(AdbOvenRack,1,2) == "BL")
ggplot(orfkpi, aes(x=Orf1abCt)) +
      labs(title="Orf1ab Ct distribution", subtitle = "Ct distribution of positive MedDX samples for previous 7 days", show_guide=F)+
  geom_histogram(binwidth=1, color="black",fill="#FAAB18")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r ngenes, echo=FALSE, fig.show="hold"}

nkpi <- subset(kpidf, NgeneCt > 0 & substr(AdbOvenRack,1,2) == "OR")
ggplot(nkpi, aes(x=NgeneCt)) +
    geom_histogram(binwidth=1, color="black", fill  = "#1380A1")+
    labs(title="N Gene Ct distribution", subtitle = "Ct distribution of positive home/satellite samples for previous 7 days", show_guide=F)+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
...
```{r sgenes, echo=FALSE, fig.show="hold"}

Skpi <- subset(kpidf, SgeneCt > 0 & substr(AdbOvenRack,1,2) == "OR" )
ggplot(Skpi, aes(x=SgeneCt)) +
  geom_histogram(binwidth=1, color="black",fill = "#990000")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
      labs(title="S Gene Ct distribution", subtitle = "Ct distribution of positive home/satellite samples for previous 7 days", show_guide=F)+
...
```{r orfgenes, echo=FALSE, fig.show="hold"}

orfkpi <- subset(kpidf, Orf1abCt > 0  & substr(AdbOvenRack,1,2) == "OR")
ggplot(orfkpi, aes(x=Orf1abCt)) +
      labs(title="Orf1ab Ct distribution", subtitle = "Ct distribution of positive home/satellite samples for previous 7 days", show_guide=F)+
  geom_histogram(binwidth=1, color="black",fill="#FAAB18")+
    stat_bin(binwidth = 1, geom="text", size=3, aes(y=..count.., label=ifelse(..count..==0,"",..count..)), position=position_stack(vjust=1.1))+
  bbc_style()
